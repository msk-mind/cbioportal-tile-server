...
async def oncokb_annotate(items: list[OncoKbMutationItem]) -> Any:
    """
    Proxy a batch of mutation annotation requests to the OncoKB API.

    Uses the public API when ONCOKB_API_TOKEN is not set (provides oncogenicity,
    mutation effect, hotspot, geneSummary, variantSummary).  With a token, uses
    the licensed API which adds treatment/therapy levels.
    """
    if not items:
        return []
...
async def oncokb_annotate_copy_number(items: list[OncoKbCopyNumberItem]) -> Any:
    """Proxy copy-number alteration annotation requests to the OncoKB API."""
    if not items:
        return []
    return await _post_oncokb(
        [item.model_dump() for item in items],
